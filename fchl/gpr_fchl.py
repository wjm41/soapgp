...
def main(task, split, n_runs, n_fold, ind):
    global rem_mat, rem_diag, max_rem

    warnings.filterwarnings('ignore')

    print('\nTraining FCHL-GP on '+task+' dataset')
    print('\nGenerating features...')
    ...
    rem_mat = np.load('/rds-d2/user/wjm41/hpc-work/kernels/FCHL/'+task+'.npy')[ind]
    ...
    class Matern32_rem(gpflow.kernels.Kernel):
            ...
            def K(self, X, X2=None, presliced=None):
                    ...
                    #return K_final
                    return tf.math.pow(K_mat,self.var)*self.mag
            ...
            def K_diag(self, X, presliced=None):
                    ...
                    return tf.math.pow(K_diag,self.var)*self.mag
    m = None
    ...
    for i in range(n_runs):
        ...
        for train_ind, test_ind in split_list:
            ...
            np.savetxt('results/fchl_'+task+'_split_'+split+'_run_'+str(j)+'_ypred.txt', y_pred)
            np.savetxt('results/fchl_'+task+'_split_'+split+'_run_'+str(j)+'_ytest.txt', y_test)
            np.savetxt('results/fchl_'+task+'_split_'+split+'_run_'+str(j)+'_ystd.txt', np.sqrt(y_var))
...
if __name__ == "__main__":
     ...
     parser.add_argument('-ind', type=int, default=0, 
                         help='FCHL kernel ind')
     args = parser.parse_args()
    
     main(args.task, args.split, args.nruns, args.nfold, args.ind)
