    ...
    for bits in bit_list:
        ...
        for i in range(n_runs):
                ...
                for train_ind, test_ind in split_list:
                    ...
                    np.savetxt('results/e3fp_'+str(bits)+'_'+task+'_split_'+split+'_run_'+str(j)+'_ypred.txt', y_pred)
                    np.savetxt('results/e3fp_'+str(bits)+'_'+task+'_split_'+split+'_run_'+str(j)+'_ytest.txt', y_test)
                    np.savetxt('results/e3fp_'+str(bits)+'_'+task+'_split_'+split+'_run_'+str(j)+'_ystd.txt', np.sqrt(y_var))
                    j+=1
...
if __name__ == "__main__":
     ...
     parser.add_argument('-bits', type=int, default=-1, 
                         help='bitsize of the fingerprint vectors')
     args = parser.parse_args()
    
     main(args.task, args.split, args.nruns, args.nfold, args.bits)
